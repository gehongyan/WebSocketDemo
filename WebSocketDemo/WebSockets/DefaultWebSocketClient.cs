...
internal class DefaultWebSocketClient : IWebSocketClient, IDisposable
{
    ...
    private const int HrTimeout = -2147012894;
    ...
    private async Task ConnectInternalAsync(string host)
    {
        ...
        foreach (KeyValuePair<string, string> header in _headers.Where(header => header.Value != null))
    ...
    private async Task OnClosed(Exception ex)
    {
        ...
        if (Closed != null) await Closed(ex);
    }
    ...
    private async Task RunAsync(CancellationToken cancelToken)
    {
        ...
        try
        {
            while (!cancelToken.IsCancellationRequested)
            {
                    ...
                    //This is a large message (likely just READY), lets create a temporary expandable stream
                {
                    ...
                    stream.Write(buffer.Array ?? Array.Empty<byte>(), 0, socketResult.Count);
                    do
                    {
                    ...
                    result = (stream.TryGetBuffer(out ArraySegment<byte> streamBuffer) ? streamBuffer.Array : stream.ToArray()) ?? Array.Empty<byte>();
                }
                else
                {
                    //Small message
                    resultCount = socketResult.Count;
                    result = buffer.Array ?? Array.Empty<byte>();
                }

                if (socketResult.MessageType == WebSocketMessageType.Text)
                {
                    string text = Encoding.UTF8.GetString(result, 0, resultCount);
                    if (TextMessage != null) await TextMessage(text).ConfigureAwait(false);
                }
                else if (BinaryMessage != null) await BinaryMessage(result, 0, resultCount).ConfigureAwait(false);
            }
        }
        catch (Win32Exception ex) when (ex.HResult == HrTimeout)
        {
        ...
        {
            //This cannot be awaited otherwise we'll deadlock when BusinessApiClient waits for this task to complete.
            Task _ = OnClosed(ex);
        }
    }
}
